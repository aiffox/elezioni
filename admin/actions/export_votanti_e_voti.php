...
//CREATE FILE----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
   ...
   header ( "Content-Disposition: attachment; filename=Votanti_e_Voti.xls" );

    echo "EMAIL\tLISTA\tCF CANDIDATO 1\tCF CANDIDATO 2\n";


    $sql = "SELECT account_votanti.email,voti.data_voto,voti.nome_lista, voti.CF_primo_candidato, voti.CF_secondo_candidato FROM voti,account_votanti WHERE account_votanti.email=voti.email ORDER BY account_votanti.email";
    $result = $conn->query($sql);
    while($row = $result->fetch_assoc()){
        echo $row['email']."\t". $row['nome_lista']."\t". $row['CF_primo_candidato']."\t".$row['CF_secondo_candidato']."\n";
    }
