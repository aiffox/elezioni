...
require '../../php/classi.php';
    ...
    if($PREPARAZIONE){
        ...
        $IDcandidato=$conn->real_escape_string($_GET['IDcandidato']);

        $sql = "SELECT * FROM candidati WHERE codice_fiscale='".$IDcandidato."'";

        //echo $sql;
        $result = $conn->query($sql);
        $dati=$result->fetch_assoc();
        if ($result->num_rows <= 0) {
            $error=$_SESSION['delete_info']= "Non e' presente alcun candidato con codice fiscale '".$IDcandidato."'. Riprova.";
            logError($error,$conn->real_escape_string((isset($_SESSION['AMM_email']) ? $_SESSION['AMM_email'] : $_SERVER['REMOTE_ADDR'])));
            //header("location: ../pannello_di_controllo.php");
            exit();
        }
    
        $sql = "DELETE FROM candidati WHERE codice_fiscale='".$IDcandidato."'";
        //echo $sql;
        $conn->query($sql);

        $_SESSION['delete_info']= "Il candidato e' stato eliminato!";
    }
    else{
